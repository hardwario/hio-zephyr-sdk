...
LOG_MODULE_REGISTER(hio_edge, CONFIG_HIO_EDGE_LOG_LEVEL);

static void event_active_work_handler(struct k_work *work)
{
	struct hio_edge *edge = CONTAINER_OF(work, struct hio_edge, event_active_work);

	k_mutex_lock(&edge->lock, K_FOREVER);

	if (edge->cb) {
		edge->cb(edge, HIO_EDGE_EVENT_ACTIVE, edge->user_data);
	}
...
{
	struct hio_edge *edge = CONTAINER_OF(work, struct hio_edge, event_inactive_work);

	k_mutex_lock(&edge->lock, K_FOREVER);

	if (edge->cb) {
		edge->cb(edge, HIO_EDGE_EVENT_INACTIVE, edge->user_data);
	}
...
static void event_timer(struct k_timer *timer_id)
{
	...
	struct hio_edge *edge = CONTAINER_OF(timer_id, struct hio_edge, event_timer);
...
static void cooldown_timer(struct k_timer *timer_id)
{
	...
	struct hio_edge *edge = CONTAINER_OF(timer_id, struct hio_edge, cooldown_timer);
...
static void gpio_handler(const struct device *port, struct gpio_callback *cb, gpio_port_pins_t pins)
{
	...
	struct hio_edge *edge = CONTAINER_OF(cb, struct hio_edge, gpio_cb);
