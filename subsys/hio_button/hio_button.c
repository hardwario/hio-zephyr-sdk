...
static struct hio_edge m_edge_ext;
...
{
	return data == &m_button_data_int ? HIO_BUTTON_CHANNEL_INT : HIO_BUTTON_CHANNEL_EXT;
}
...
int hio_button_set_event_cb(hio_button_event_cb cb, void *user_data)
{
	...
	ret = hio_edge_watch(&m_edge_ext);
	if (ret) {
		LOG_ERR("Call `hio_edge_watch` (BUTTON_EXT) failed: %d", ret);
		return ret;
	}
...
static int init(void)
{
	...
	ret = hio_edge_init(&m_edge_ext, &m_button_ext, false);
	if (ret) {
		LOG_ERR("Call `hio_edge_init` (BUTTON_EXT) failed: %d", ret);
		return ret;
	}

	ret = hio_edge_set_callback(&m_edge_ext, edge_event_cb, NULL);
	if (ret) {
		LOG_ERR("Call `hio_edge_set_callback` (BUTTON_EXT) failed: %d", ret);
		return ret;
	}

	ret = hio_edge_set_cooldown_time(&m_edge_ext, 10);
	if (ret) {
		LOG_ERR("Call `hio_edge_set_cooldown_time` (BUTTON_EXT) failed: %d", ret);
		return ret;
	}

	ret = hio_edge_set_active_duration(&m_edge_ext, 10);
	if (ret) {
		LOG_ERR("Call `hio_edge_set_active_duration` (BUTTON_EXT) failed: %d", ret);
		return ret;
	}

	ret = hio_edge_set_inactive_duration(&m_edge_ext, 10);
	if (ret) {
		LOG_ERR("Call `hio_edge_set_inactive_duration` (BUTTON_EXT) failed: %d", ret);
		return ret;
	}
