...
LOG_MODULE_REGISTER(hio_cloud_shell, CONFIG_HIO_CLOUD_LOG_LEVEL);
...
static int cmd_state(const struct shell *shell, size_t argc, char **argv)
{
	...
	bool initialized;
	ret = hio_cloud_is_initialized(&initialized);
	if (ret) {
		shell_error(shell, "hio_cloud_is_initialized failed: %d", ret);
		return ret;
	}
...
static int cmd_firmware_download(const struct shell *shell, size_t argc, char **argv)
{
	...
	ret = hio_cloud_firmware_update(argv[1]);
	if (ret) {
...
static int cmd_poll(const struct shell *shell, size_t argc, char **argv)
{
	...
	if (ret) {
		shell_error(shell, "hio_cloud_poll_immediately failed: %d", ret);
		return ret;
	}
