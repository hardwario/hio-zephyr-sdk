...
LOG_MODULE_REGISTER(hio_cloud, CONFIG_HIO_CLOUD_LOG_LEVEL);
...
static hio_cloud_cb m_user_cb;
static void *m_user_data;
static struct hio_cloud_options *m_options = NULL;
static struct hio_cloud_session m_session;
...
static int process_downlink(struct hio_buf *buf, struct hio_buf *upbuf)
{
	...
	size_t len = hio_buf_get_used(buf);
	if (!len) {
	...
	case DL_SET_SESSION:
		...
		ret = hio_cloud_msg_unpack_set_session(buf, &m_session);
		if (ret) {
			LOG_ERR("Call `hio_cloud_msg_unpack_set_session` failed: %d", ret);
		...
		ret = hio_rtc_set_ts(m_session.timestamp);
		if (ret) {
			LOG_ERR("Call `hio_rtc_set_ts` failed: %d", ret);
			return ret;
		}

		k_mutex_lock(&m_lock_state, K_FOREVER);

		m_state_last_seen_ts = k_uptime_get();
		ret = hio_rtc_get_ts(&m_state_last_seen_ts);
	...
	case DL_SET_TIMESTAMP:
		...
		uint64_t ts;
		ret = hio_cloud_msg_unpack_set_timestamp(buf, &ts);
		if (ret) {
			LOG_ERR("Call `hio_cloud_msg_unpack_set_timestamp` failed: %d", ret);
			return ret;
		}
		ret = hio_rtc_set_ts(ts);
		if (ret) {
			LOG_ERR("Call `hio_rtc_set_ts` failed: %d", ret);
			return ret;
		}
	...
	case DL_DOWNLOAD_CONFIG:
		...
		ret = hio_cloud_msg_unpack_config(buf, &config);
		if (ret) {
			LOG_ERR("Call `hio_cloud_msg_unpack_config` failed: %d", ret);
			return ret;
		}
		ret = hio_cloud_process_dlconfig(&config);
		if (ret) {
			LOG_ERR("Call `hio_cloud_process_dlconfig` failed: %d", ret);
			return ret;
		}
		break;
	case DL_DOWNLOAD_DATA:
		...
		if (m_user_cb) {
			union hio_cloud_event_data data = {
				.recv.buf = &p[1],
				.recv.len = len - 1,
			};

			m_user_cb(HIO_CLOUD_EVENT_RECV, &data, m_user_data);
		} else {
	...
	case DL_DOWNLOAD_SHELL:
		...
		struct hio_cloud_msg_dlshell msg;
		ret = hio_cloud_msg_unpack_dlshell(buf, &msg);
		if (ret) {
			LOG_ERR("Call `hio_cloud_msg_unpack_dlshell` failed: %d", ret);
			return ret;
		}
		ret = hio_cloud_process_dlshell(&msg, upbuf);
		if (ret) {
			LOG_ERR("Call `hio_cloud_process_dlshell` failed: %d", ret);
			return ret;
		}
		break;
	case DL_DOWNLOAD_FIRMWARE:
		LOG_DBG("Received firmware");
		struct hio_cloud_msg_dlfirmware firmware;
		ret = hio_cloud_msg_unpack_dlfirmware(buf, &firmware);
		if (ret) {
			LOG_ERR("Call `hio_cloud_msg_unpack_dlfirmware` failed: %d", ret);
			return ret;
		}
		ret = hio_cloud_process_dlfirmware(&firmware, upbuf);
		if (ret) {
			LOG_ERR("Call `hio_cloud_process_dlfirmware` failed: %d", ret);
			return ret;
		}
		break;
	default:
...
{
	int ret = hio_cloud_recv();
	if (ret) {
		LOG_ERR("Call `hio_cloud_recv` failed: %d", ret);
		return;
	}
}
...
static int transfer(struct hio_buf *buf)
{
	...
	bool has_downlink = hio_buf_get_used(buf) == 0;
	...
	if (has_downlink) {
		...
		struct hio_buf upbuf = {
			.mem = hio_buf_get_mem(buf) + hio_buf_get_used(buf),
			.size = hio_buf_get_free(buf),
			.len = 0,
		};
...
static int create_session(void)
{
	...
	ret = hio_cloud_msg_pack_create_session(&m_transfer_buf);
	if (ret) {
		LOG_ERR("Call `hio_cloud_msg_pack_create_session` failed: %d", ret);
...
static int upload_decoder(void)
{
	...
	if (m_session.decoder_hash != m_options->decoder_hash) {
		...
		ret = hio_cloud_msg_pack_decoder(&m_transfer_buf, m_options->decoder_hash,
						 m_options->decoder_buf, m_options->decoder_len);
		if (ret) {
			LOG_ERR("Call `hio_cloud_msg_pack_decoder` failed: %d", ret);
			k_mutex_unlock(&m_lock);
			return ret;
		}
...
static int upload_encoder(void)
{
	...
	if (m_session.encoder_hash != m_options->encoder_hash) {
		...
		ret = hio_cloud_msg_pack_encoder(&m_transfer_buf, m_options->encoder_hash,
						 m_options->encoder_buf, m_options->encoder_len);
		if (ret) {
			LOG_ERR("Call `hio_cloud_msg_pack_encoder` failed: %d", ret);
			k_mutex_unlock(&m_lock);
			return ret;
		}
...
static int upload_config(void)
{
	...
	ret = hio_cloud_msg_pack_config(&m_transfer_buf);

	if (ret) {
		LOG_ERR("Call `hio_cloud_msg_pack_config` failed: %d", ret);
		k_mutex_unlock(&m_lock);
		return ret;
	}
	...
	ret = hio_cloud_msg_get_hash(&m_transfer_buf, &hash);
	if (ret) {
		LOG_ERR("Call `hio_cloud_msg_get_hash` failed: %d", ret);
		k_mutex_unlock(&m_lock);
		return ret;
	}
...
static int firmware_confirmed(void)
{
	...
	hio_cloud_uuid_t fuid;

	ret = hio_cloud_util_get_firmware_update_id(fuid);
	...
	ret = hio_cloud_msg_pack_firmware(&m_transfer_buf, &upfirmware);
	if (ret) {
...
int hio_cloud_init(struct hio_cloud_options *options)
{
	...
	ret = hio_info_get_serial_number_uint32(&serial_number);
	if (ret) {
		LOG_ERR("Call `hio_info_get_serial_number_uint32` failed: %d", ret);
		return ret;
	}

	const char *claim_token_str;
	ret = hio_info_get_claim_token(&claim_token_str);
	if (ret) {
		LOG_ERR("Call `hio_info_get_claim_token` failed: %d", ret);
		return ret;
	}
...
int hio_cloud_send(const void *buf, size_t len)
{
	...
	ret = hio_buf_append_u8(&m_transfer_buf, UL_UPLOAD_DATA);
	if (ret) {
	...
	ret = hio_buf_append_u64_be(&m_transfer_buf, m_options->decoder_hash);
	if (ret) {
		LOG_ERR("Call `hio_buf_append_u64` failed: %d", ret);
		k_mutex_unlock(&m_lock);
		return ret;
	}

	ret = hio_buf_append_mem(&m_transfer_buf, buf, len);
	if (ret) {
...
int hio_cloud_firmware_update(const char *firmware)
{
	...
	ret = hio_cloud_msg_pack_firmware(&m_transfer_buf, &upfirmware);
	if (ret) {
		LOG_ERR("Call `hio_cloud_msg_pack_firmware` failed: %d", ret);
		k_mutex_unlock(&m_lock);
		return ret;
	}
