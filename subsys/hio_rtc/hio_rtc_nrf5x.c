/* hio_rtc implementation for the nRF52/nRF91 series (RTC peripheral).
 *
 * Standalone implementation selected by CMake when CONFIG_SOC_SERIES_NRF54LX is
 * NOT set. Shares only the public API in <hio/hio_rtc.h> with the nRF54 variant
 * (hio_rtc_nrf54.c).
 *
 * The RTC runs at 8 Hz (prescaler 4095 over 32768 Hz) and a /8 software
 * prescaler turns the tick interrupt into a 1 Hz update of the wall-clock.
 */
