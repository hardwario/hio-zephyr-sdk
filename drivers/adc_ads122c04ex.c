...
// #include "../include/drivers/adc_ads122c04ex.h"
#include <drivers/adc_ads122c04ex.h>

#define DT_DRV_COMPAT ti_ads122c04ex

// TODO log level with BLE?
LOG_MODULE_REGISTER(ads122c04ex, CONFIG_ARCH_LOG_LEVEL);
...
static int ads122c04_read(const struct device *dev, int32_t *result)
{
	int ret;
...
static int ads122c04_set_mux(const struct device *dev, int mux)
{
	int ret;
	...
	get_data(dev)->reg_config_0 &= 0x0F;
	get_data(dev)->reg_config_0 |= mux << POS_CONFIG_0_MUX;
...
__unused static int ads122c04_init(const struct device *dev)
{
	int ret;
	...
	get_data(dev)->reg_config_1 |= 4 << POS_CONFIG_1_DR;
...
__unused static const struct adc_ads122c04_driver_api ads122c04_driver_api = {
	.read = ads122c04_read,
	.set_mux = ads122c04_set_mux,
};
...
#define ADS122C04_INIT(n)                                                                          \
	...
	DEVICE_DT_INST_DEFINE(n, ads122c04_init, NULL, &inst_##n##_data, &inst_##n##_config,       \
			      POST_KERNEL, 51, &ads122c04_driver_api);
